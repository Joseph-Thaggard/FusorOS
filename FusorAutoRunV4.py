...
def execute_enhanced_program(path: str):
    """Execute program with full variable support"""
    ...
    try:
        ...
        for func_name, file_path in autorun_config["function_maps"].items():
            if os.path.exists(file_path):
                func_commands = load_program_text(file_path)
        ...
        log_event(f"Loaded {len(functions)} functions, {len(commands)} main commands")
